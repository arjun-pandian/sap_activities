...
def run_pks_script():
    ...
    take_and_save_screenshot("pks", "1_sm12")
    ...
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("pks", "2_sm13")

    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsm21"
    session.findById("wnd[0]").sendVKey (0)
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("pks", "3_sm21")
    ...
    take_and_save_screenshot("pks", "4_st22")
    #'''
    ...
    try:
        ...
        take_and_save_screenshot("pks", "5_sm37_1")
    ...
    try:
        ...
        session.findById("wnd[0]").sendVKey (2)
        session.findById("wnd[0]").sendVKey (41) 
        take_and_save_screenshot("pks", "6_sm37_2")
    except:
        take_and_save_screenshot("pks", "6_sm37_2")
    #'''
    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsm51"
    session.findById("wnd[0]").sendVKey (0)
    take_and_save_screenshot("pks", "7_sm51")
    ...
    session.findById("wnd[0]").sendVKey (0)
    take_and_save_screenshot("pks", "8_sm66")

    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsmq1"
    session.findById("wnd[0]").sendVKey (0)
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("pks", "9_smq1")
    ...
    take_and_save_screenshot("pks", "10_stms_1")
    ...
    # Write values into cells B9 to I9
    for index, value in enumerate(values):
        ws.cell(row=9, column=index+2, value=value) 
