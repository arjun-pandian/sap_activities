...
def run_pr5_script():
    ...
    take_and_save_screenshot("pr5", "1_sm12")
    ...
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("pr5", "2_sm13")

    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsm21"
    session.findById("wnd[0]").sendVKey (0)
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("pr5", "3_sm21")
    ...
    take_and_save_screenshot("pr5", "4_st22")
    #'''
    ...
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("pr5", "5_sm37_1")
    try :
        ...
        for x in range(12, 30):
            try:
                ...
                if "Summary" in text:
                    job_time_details = session.findById(f"wnd[0]/usr/lbl[91,{x}]").Text
                    sm37_total_job_time = job_time_details.replace(' ', '').replace('.', '').replace(',','')
    ...
    try :
        ...
        session.findById("wnd[0]").sendVKey (2)
        session.findById("wnd[0]").sendVKey (41) 
        take_and_save_screenshot("pr5", "6_sm37_2")
    except :
        take_and_save_screenshot("pr5", "6_sm37_2")
    #'''
    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsm51"
    session.findById("wnd[0]").sendVKey (0)
    take_and_save_screenshot("pr5", "7_sm51")
    ...
    session.findById("wnd[0]").sendVKey (0)
    take_and_save_screenshot("pr5", "8_sm66")

    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsmq1"
    session.findById("wnd[0]").sendVKey (0)
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("pr5", "9_smq1")

    session.findById("wnd[0]/tbar[0]/okcd").text = "/nstms"
    session.findById("wnd[0]").sendVKey (0)
    session.findById("wnd[0]/tbar[1]/btn[5]").press()
    session.findById("wnd[0]/usr/lbl[2,11]").setFocus()
    hovered_cell = session.findById("wnd[0]/usr/lbl[2,11]").Text
    take_and_save_screenshot("pr5", "10_stms_1")
    ...
    take_and_save_screenshot("pr5", "17_tablespace")
    ...
    session.findById("wnd[0]/usr/tabsTS_TABSTRIP/tabpTS_TABSTRIP_TAB1_/ssub221_SCA:SAPLS_ORA_COCKPIT_5:0221/cntlCC_TAB1_0221/shellcont/shell").contextMenu()
    session.findById("wnd[0]/usr/tabsTS_TABSTRIP/tabpTS_TABSTRIP_TAB1_/ssub221_SCA:SAPLS_ORA_COCKPIT_5:0221/cntlCC_TAB1_0221/shellcont/shell").selectContextMenuItem ("&FILTER")
    session.findById("wnd[1]/usr/ssub%_SUBSCREEN_FREESEL:SAPLSSEL:1105/txt%%DYN002-LOW").text = "91"
    session.findById("wnd[1]/usr/ssub%_SUBSCREEN_FREESEL:SAPLSSEL:1105/txt%%DYN002-HIGH").text = "100"
    session.findById("wnd[1]/tbar[0]/btn[0]").press()
    ...
    # Write values into cells B10 to I10
    for index, value in enumerate(values):
        ws.cell(row=10, column=index+2, value=value) 
