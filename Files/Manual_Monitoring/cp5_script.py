...
def run_cp5_script():
    ...
    take_and_save_screenshot("cp5", "1_sm12")
    ...
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("cp5", "2_sm13")

    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsm21"
    session.findById("wnd[0]").sendVKey (0)
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("cp5", "3_sm21")
    ...
    take_and_save_screenshot("cp5", "4_st22")
    ...
    try :
        ...
        take_and_save_screenshot("cp5", "5_sm37_1")
    ...
    try :
        ...
        session.findById("wnd[0]").sendVKey (2)
        session.findById("wnd[0]").sendVKey (41) 
        take_and_save_screenshot("cp5", "6_sm37_2")
    except :
        take_and_save_screenshot("cp5", "6_sm37_2")
    
    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsm51"
    session.findById("wnd[0]").sendVKey (0)
    take_and_save_screenshot("cp5", "7_sm51")
    ...
    session.findById("wnd[0]").sendVKey (0)
    take_and_save_screenshot("cp5", "8_sm66")

    session.findById("wnd[0]/tbar[0]/okcd").text = "/nsmq1"
    session.findById("wnd[0]").sendVKey (0)
    session.findById("wnd[0]/tbar[1]/btn[8]").press()
    take_and_save_screenshot("cp5", "9_smq1")
    ...
    take_and_save_screenshot("cp5", "10_stms_1")
    ...
    if hovered_cell == "CP5":
        ...
        for x in range(6, 30):
            try:
                ...
                if "CP5" in text:
                    take_and_save_screenshot("cp5", "9_stms_1")
                    session.findById("wnd[0]/tbar[1]/btn[18]").press()
                    session.findById("wnd[1]/usr/btnSPOP-OPTION2").press()
                    print("CP5 found")
                    break  
    ...
    take_and_save_screenshot("cp5", "17_tablespace")
    ...
    # Write values into cells B7 to I7
    for index, value in enumerate(values):
        ws.cell(row=7, column=index+2, value=value) 
